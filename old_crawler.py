...
from old_image_helper import img2hash


def filename(url):
    return "ebay_" + hashlib.sha256(url.encode("utf-8")).hexdigest() + ".html"
...
def web_crawler(url):
    # https://www.kleinanzeigen.de/s-preis:20:/l%C3%B6tstation/k0
    # https://www.kleinanzeigen.de/s-preis:20:/seite:2/l%C3%B6tstation/k0
    if 0 == url.count(":/seite:"):
        url = re.sub( r"/s-preis:20:/","/s-preis:20:/seite:1/", url)
    i = 1
    while 1 == i or (data.count(":/seite:" + str(i)) > 0 and len(data) > 1 and i < 4 ):
        url = re.sub(r":/seite:[0-9]+", ":/seite:" + str(i), url)
        data, tags, image_tags = web_crawler_internal(url)
        print( url + " data: " + str(len(data)) + " tags: " + str(len(tags)) + "  image_tags: " + str(len(image_tags)))
        i = i + 1
    return data, tags, image_tags


def web_crawler_internal(url):
    data = read( url)
        ...
        if len(descr) > 0 and len(abs_url) > 0 and not lnk is None :
            ...
            for word in (txt + " " + descr[0].text).split(" "):
                ...
                if len(word)>1:
                    ...
                    img_hash[ linkElems[3] ] =  img2hash( abs_url )
...
def scanAndSave(url):
    ...
    print("")
    with open('cache/data.csv', 'w', encoding='utf8') as file:
        for entry in sortedEntries:
            ...
            if "Vorschau" == entry[0]:
                ...
                for item in sortedItems:
                    ...
                    # print("  " + prices[item] + " " + images[item] ) # + " " + links[item])
                    # print("  " + prices[item] + " " + images[item] + " " + links[item])
                    prices[item] = prices[item].replace("€", "")
                    file.write("  " + prices[item] + " " + images[item] + " " + links[item] + " " + img_hash[item] + "\n")
                None
                # print("")
